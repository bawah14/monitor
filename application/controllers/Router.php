...
class router extends CI_Controller {
	...
	public function input_ketersediaan(){
		...
		foreach ($data['komoditi'] as $key => $value) {
			# code...
			$data["ketersediaan"][$value->nama_komoditi] = $this->ketersediaan->manual_query('select * from ketersediaan where id_komoditi = '.$value->id_komoditi." and minggu_ketersediaan = ".$data['webconfig'][0]->minggu_ketersediaan_webconfig." and bulan_ketersediaan = ".$data['webconfig'][0]->bulan_ketersediaan_webconfig." and tahun_ketersediaan = ".$data['webconfig'][0]->tahun_ketersediaan_webconfig." and pasar_ketersediaan = ".$_SESSION['id_pasar'])->result();
		}
	...
	public function edit_ketersediaan(){
		$this->load->model('pasar');
		$this->load->model('komoditi');
		$this->load->model('ketersediaan');
		$this->load->model('webconfig');
		...
		if (isset($_GET['hapus'])) {
			# code...
			$this->hapusdataketersediaan($_GET['hapus']);
			$data['pesan'] = "data dihapus";
		}
	...
	public function handlereditketersediaan(){
		$this->load->model('ketersediaan');
		// print_r($_POST);
		$this->ketersediaan->manual_query("update ketersediaan set nama_pedagang = '".$_POST['nama_pedagang']."' , jumlah_ketersediaan = '".$_POST['jumlah_ketersediaan']."' where id_ketersediaan = ".$_POST['id']);
		// echo "update ketersediaan set nama_pedagang = '".$_POST['nama_pedagang']."' , jumlah_ketersediaan = '".$_POST['jumlah_ketersediaan']."' where id_ketersediaan = ".$_POST['id'];
		// die();
		redirect(base_url("/index.php/router/index/edit_ketersediaan?pesan=data ".$_POST['nama_pedagang']." telah diubah&minggu=".$_POST['minggu']."&bulan=".$_POST['bulan']."&tahun=".$_POST['tahun']."&pasar=".$_POST['pasar']));
	}

	public function kelola_ketersediaan_new(){
		$this->load->model('pasar');
		$this->load->model('komoditi');
		$this->load->model('ketersediaan');
		$this->load->model('webconfig');
		...
		$data['ketersediaan'] = null;
		$data['webconfig'] = $this->webconfig->manual_query('select * from webconfig')->result();
		if (isset($_GET['minggu']) && isset($_GET['bulan']) && isset($_GET['tahun'])) {
			# code...
			foreach ($data['pasar'] as $key => $value) {
				# code...
				$data['ketersediaan'][$value->nama_pasar] = $this->ketersediaan->manual_query('select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = '.$value->id_pasar.' and minggu_ketersediaan = '.$_GET['minggu'].' and bulan_ketersediaan = '.$_GET['bulan'].' and tahun_ketersediaan = '.$_GET['tahun'].' group by ketersediaan.id_komoditi')->result();
				$data['perpasar'][$value->nama_pasar] = $this->ketersediaan->manual_query('select * from ketersediaan where pasar_ketersediaan = "'.$value->id_pasar.'" and minggu_ketersediaan = '.$_GET['minggu']." and bulan_ketersediaan = ".$_GET['bulan']." and tahun_ketersediaan = ".$_GET['tahun']." order by id_komoditi")->result();
				$data['query'][$value->nama_pasar] = 'select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = $value->id_pasar and minggu_ketersediaan = '.$_GET['minggu'].' and bulan_ketersediaan = '.$_GET['bulan'].' and tahun_ketersediaan = '.$_GET['tahun'].' group by ketersediaan.id_komoditi';
			}
		}
				// $this->ketersediaan->manual_query('select * from ketersediaan');
		return $data;
	...
	public function lihat_harga(){
		...
		if ($this->input->post("pasar")!=null) {
			# code...
			$query_pasar =" and pasar.id_pasar = ".$this->input->post("pasar");
			$data['pasar'] = $this->input->post("pasar");
		}else{
			$query_pasar = "";
		}
		foreach ($data['allkomoditi']->result() as $value) {
	...
	public function handlerexportketersediaan(){
		...
		foreach ($data['pasar'] as $key => $value) {
			...
			for ($i=1; $i < 6; $i++) { 
				# code...
				$data['ketersediaan'][$i][$value->nama_pasar] = $this->ketersediaan->manual_query('select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi from ketersediaan inner join komoditi on komoditi.id_komoditi = ketersediaan.id_komoditi where bulan_ketersediaan = '.$data['bulan'].' and tahun_ketersediaan = '.$data['tahun']." and minggu_ketersediaan = ".$i);
			}
		}
		// print_r($data['ketersediaan']);
		// die();
	...
	public function grafik_ketersediaan(){
		...
		if (isset($_GET['bulan']) && isset($_GET['tahun']) && isset($_GET['bulan'] )) {
			for ($i=1; $i < 6 ; $i++){
				$data["perminggu"][$i] = $this->ketersediaan->manual_query('select sum(jumlah_ketersediaan) as total from ketersediaan where id_komoditi = '.$_GET['komoditi'].' and minggu_ketersediaan = '.$i." and bulan_ketersediaan = ".$_GET['bulan']." and tahun_ketersediaan = ".$_GET['tahun'])->result();		}
			}
		return $data;
	}
	...
	public function hapusdataketersediaan($id){
		$this->load->model('ketersediaan');
		$this->ketersediaan->manual_query("delete from ketersediaan where id_ketersediaan = ".$id);
	}

	public function handlerinputketersediaanpetugas(){
		// print_r($_POST);
		// die();
		$this->load->model('ketersediaan');
		if (!isset($_POST['tanggal'])) {
		...
		$tahun = $_POST['tahun'];
		// $this->ketersediaan->manual_query('delete from ketersediaan where tanggal_ketersediaan = '.$tanggal.' and pasar_ketersediaan = '.$pasar);
		$this->ketersediaan->input_ketersediaan($_POST['id_komoditi'],$tanggal,$_POST['jumlah_ketersediaan'],1,$_POST['nama_pedagang'],$_POST['pasar'],0,$_POST['asal_ketersediaan'],$minggu,$bulan,$tahun);
		redirect(base_url("/index.php/router/index/edit_ketersediaan"));
	}

	public function handlerlaporanperpasar(){
		// print_r($_POST);
		// die();
		$this->load->model('pasar');
		$this->load->model('komoditi');
		...
		if (  isset($_POST['bulan']) && isset($_POST['tahun'])) {
			...
			foreach ($data['pasar'] as $key => $value) {
				# code...
				$data['ketersediaan'][$value->nama_pasar] = $this->ketersediaan->manual_query('select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = '.$value->id_pasar.' and  bulan_ketersediaan = '.$_POST['bulan'].' and tahun_ketersediaan = '.$_POST['tahun'].' group by ketersediaan.id_komoditi')->result();
				$data['perpasar'][$value->nama_pasar] = $this->ketersediaan->manual_query('select * from ketersediaan where pasar_ketersediaan = "'.$value->id_pasar.'" '." and bulan_ketersediaan = ".$_POST['bulan']." and tahun_ketersediaan = ".$_POST['tahun']." order by id_komoditi")->result();
				$data['query'][$value->nama_pasar] = 'select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = $value->id_pasar and bulan_ketersediaan = '.$_POST['bulan'].' and tahun_ketersediaan = '.$_POST['tahun'].' group by ketersediaan.id_komoditi';
			}
		}
		$this->load->view('export-ketersediaan',$data);
	...
	public function handlerlaporanperpasartahunan(){
		// print_r($_POST);
		// die();
		$this->load->model('pasar');
		$this->load->model('komoditi');
		...
		if (  isset($_POST['tahun'])) {
			...
			foreach ($data['pasar'] as $key => $value) {
				# code...
				$data['ketersediaan'][$value->nama_pasar] = $this->ketersediaan->manual_query('select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = '.$value->id_pasar.' and tahun_ketersediaan = '.$_POST['tahun'].' group by ketersediaan.id_komoditi')->result();
				$data['perpasar'][$value->nama_pasar] = $this->ketersediaan->manual_query('select * from ketersediaan where pasar_ketersediaan = "'.$value->id_pasar.'" '." and tahun_ketersediaan = ".$_POST['tahun']." order by id_komoditi")->result();
				$data['query'][$value->nama_pasar] = 'select sum(jumlah_ketersediaan) as jumlah,komoditi.nama_komoditi,pasar.nama_pasar from ketersediaan inner join komoditi on ketersediaan.id_komoditi = komoditi.id_komoditi INNER join pasar on ketersediaan.pasar_ketersediaan = pasar.id_pasar where pasar_ketersediaan = $value->id_pasar and tahun_ketersediaan = '.$_POST['tahun'].' group by ketersediaan.id_komoditi';
			}
		}
		$this->load->view('export-ketersediaan',$data);
	...
	public function handlerlaporanhargaperkomoditi(){
		// print_r($_POST);
		$this->load->model('pasar');
		$this->load->model('harga');
		$this->load->model("komoditi");
		$data['bulan'] = $_POST['bulan'];
		$data['tahun'] = $_POST['tahun'];
		$data['komoditi'] = $this->komoditi->manual_query('select * from komoditi where id_komoditi = '.$_POST['komoditi'])->result();
		$data['pasar'] = $this->pasar->manual_query('select * from pasar order by nama_pasar')->result();
		foreach ($data['pasar'] as $key => $value) {
			# code...
			$data['harga'][$value->nama_pasar] = $this->harga->manual_query("select day(tanggal_harga) as tanggal , harga_harga as harga from harga where id_komoditi = ".$_POST['komoditi']." and month(tanggal_harga) = ".$_POST['bulan']." and year(tanggal_harga) = ".$_POST['tahun']." and id_pasar = ".$value->id_pasar)->result();
		}
		// print_r($data['harga']);
		// die();
		$this->load->view('export-harga-perkomoditi',$data);

	}

	public function handlerlaporanketersediaan(){
		// $this->load->model('pasar');
		$this->load->model('komoditi');
		$this->load->model('ketersediaan');
		// $this->load->model('pasar');
		...
		// print_r($data["perminggu"]);
		$this->load->view('export-ketersediaan-bulanan',$data);

	}

	public function lihat_ketersediaan(){
		$data = null;
		$this->load->model('komoditi');
		$this->load->model('ketersediaan');
		// $this->load->model('pasar');
